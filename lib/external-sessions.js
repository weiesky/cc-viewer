...
/**
 * Load external roots from env + preferences. Returns [{ index, path }] with dedup.
 */
export function loadRoots({ envValue, preferenceRoots } = {}) {
  const fromEnv = parseRootsEnv(envValue);
  const fromPrefs = (preferenceRoots || [])
    .filter(p => typeof p === 'string')
    .map(expandHome)
    .filter(isAbsolute);
  const seen = new Set();
  const result = [];
  for (const p of [...fromEnv, ...fromPrefs]) {
  ...
  for (const name of listValidSlugDirs(scopesDir)) {
    ...
    if (!data) {
      scopes.push({
        ...
        kind: 'unknown', meta: [], updatedAt: null,
      });
      continue;
    }
    scopes.push({
  ...
  for (const name of listValidSlugDirs(sessionsDir)) {
    ...
    if (!data) {
      sessions.push({
        ...
        parentSessionId: null, meta: [], updatedAt: null,
      });
      continue;
    }
    sessions.push({
