...
// Parses CCV_EXTERNAL_SESSION env and writes session.json skeleton + endedAt.
...
export function getExternalSession() {
  ...
  _parsed = {
    ...
    parentSessionId: typeof obj.parentSessionId === 'string' ? obj.parentSessionId : null,
    meta: Array.isArray(obj.meta) ? obj.meta : [],
  };
  return _parsed;
}

/**
 * Write session.json skeleton on proxy start.
 * No-op (returns false) if env absent or session.json already exists (resume case).
 */
export function writeSessionSkeleton() {
  ...
  const skeleton = {
    ...
    startedAt: now,
    endedAt: null,
    parentSessionId: ext.parentSessionId,
    meta: ext.meta,
    updatedAt: now,
  };
  try {
...
/**
 * Mark session ended (fill endedAt) when proxy exits. Idempotent.
 */
