...
// appSecret now lives in the credential vault (cleared from preferences.json); appId (low-sensitivity cred) stays base64.
...
describe('feishu save / load roundtrip', () => {
  ...
  it('roundtrips (plaintext in memory; appId base64 on disk; appSecret in the vault), keeps region plaintext', () => {
    reset();
    ...
    assert.equal(onDisk.feishu.appSecret, '', 'secret field cleared on disk (vault-backed)');
    assert.equal(onDisk.feishu.appId, Buffer.from('cli_abc', 'utf-8').toString('base64'), 'low-sensitivity cred stays base64');
    ...
    assert.ok(creds.creds['im-secret:feishu.appSecret'], 'secret lives in the vault');
  });
