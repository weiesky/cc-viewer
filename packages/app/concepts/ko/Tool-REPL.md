...
- 기본적으로 꺼져 있습니다 — 활성화하려면 `CLAUDE_CODE_REPL=true`를 설정하십시오.
- 터미널(`cli`) 및 claude.ai(`remote`) 세션에서는 서버 측 기능 플래그가 이를 활성화할 수도 있습니다.
- 꺼져 있으면 REPL은 모델의 도구 목록에서 숨겨집니다. 켜져 있으면 `Read`, `Glob`, `Grep`, `Bash`, `PowerShell`, `NotebookEdit`가 REPL 축약형으로 대체됩니다.
