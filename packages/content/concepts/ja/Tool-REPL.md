...
- デフォルトではオフです — 有効にするには `CLAUDE_CODE_REPL=true` を設定します。
- ターミナル (`cli`) および claude.ai (`remote`) セッションでは、サーバーサイドの機能フラグによって有効になることもあります。
- オフの場合、REPL はモデルのツールリストから隠されます。オンの場合、`Read`、`Glob`、`Grep`、`Bash`、`PowerShell`、`NotebookEdit` は REPL の短縮形に置き換えられます。
