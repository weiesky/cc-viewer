/**
 * Unit tests for the shared client-safe module @ccv/core/session-boundary
 * (wire-v2 S1: verbatim moves from the legacy src/utils/clearCheckpoint.js and
 * sessionMerge.js; @ccv/core extraction: now packages/core/src/).
 *
 * Boundary-predicate behavior is already covered end-to-end by
 * packages/core/test/clearCheckpoint.test.js (through the @ccv/core/clearCheckpoint
 * re-export shell) and test/session-boundary-parity.test.js; this file (a) pins
 * the canonical import path, (b) adds the first DIRECT tests for
 * findReverseAnchor, which was previously module-private in sessionMerge.js.
 */
...
describe('canonical import path exposes the full shared surface', () => {
  ...
  it('@ccv/core/clearCheckpoint re-exports the same function objects', async () => {
    const shell = await import('../src/clearCheckpoint.js');
