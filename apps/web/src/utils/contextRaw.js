...
// step 节点输出 [userMsg, ...systemMsgs, assistantMsg] 原始消息切片（rawSystem 为
// 折叠进本 step 的 mid-conversation system 消息，按会话顺序位于 user 与 assistant 之间）；
// 当前步 assistant 若由 response 覆盖（见 ContextTab steps useMemo），rawAssistant 即完整 response body。
export function buildContextItemRawText(item) {
  if (!item) return '';
  const raw = item.isStep
