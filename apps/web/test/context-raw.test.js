...
describe('buildContextItemRawText', () => {
  ...
  it('step 节点（有 assistant）输出 [user, assistant] 原始消息切片', () => {
    const userMsg = { role: 'user', content: 'hi' };
    const assistantMsg = { role: 'assistant', content: [{ type: 'text', text: 'hello' }] };
    const out = buildContextItemRawText({ isStep: true, rawUser: userMsg, rawAssistant: assistantMsg });
    assert.equal(out, JSON.stringify([userMsg, assistantMsg], null, 2));
  });

  it('step 节点（无 assistant）只输出 [user]', () => {
    const userMsg = { role: 'user', content: 'hi' };
    const out = buildContextItemRawText({ isStep: true, rawUser: userMsg, rawAssistant: null });
    assert.equal(out, JSON.stringify([userMsg], null, 2));
  });

  it('末步 response 覆盖后 assistant 部分为完整 response body', () => {
    const userMsg = { role: 'user', content: 'hi' };
    const responseBody = { id: 'msg_1', model: 'claude', content: [{ type: 'text', text: 'ok' }], usage: { output_tokens: 3 } };
    // 模拟 ContextTab steps useMemo 的覆盖结果：rawAssistant = response body
    const out = buildContextItemRawText({ isStep: true, rawUser: userMsg, rawAssistant: responseBody });
    assert.equal(out, JSON.stringify([userMsg, responseBody], null, 2));
  });

  it('step with rawSystem → [user, ...system, assistant] slice order', () => {
    const userMsg = { role: 'user', content: 'hi' };
    const sysMsg = { role: 'system', content: 'gentle reminder' };
    const assistantMsg = { role: 'assistant', content: [{ type: 'text', text: 'ok' }] };
    const out = buildContextItemRawText({ isStep: true, rawUser: userMsg, rawSystem: [sysMsg], rawAssistant: assistantMsg });
    assert.equal(out, JSON.stringify([userMsg, sysMsg, assistantMsg], null, 2));
  });

  it('last-step response override keeps folded system messages', () => {
    // Mirrors ContextTab steps useMemo: {...last} preserves rawSystem while
    // rawAssistant is replaced by the full response body.
    const userMsg = { role: 'user', content: 'hi' };
    const sysMsg = { role: 'system', content: 'gentle reminder' };
    const responseBody = { id: 'msg_1', content: [{ type: 'text', text: 'ok' }] };
    const out = buildContextItemRawText({ isStep: true, rawUser: userMsg, rawSystem: [sysMsg], rawAssistant: responseBody });
  ...
  it('rawSystem null/absent → output identical to pre-rawSystem shape (old logs)', () => {
    const userMsg = { role: 'user', content: 'hi' };
    const assistantMsg = { role: 'assistant', content: [{ type: 'text', text: 'ok' }] };
    const withNull = buildContextItemRawText({ isStep: true, rawUser: userMsg, rawSystem: null, rawAssistant: assistantMsg });
    const withoutKey = buildContextItemRawText({ isStep: true, rawUser: userMsg, rawAssistant: assistantMsg });
