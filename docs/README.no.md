...
* **Innebygde forhåndsinnstillinger er aktive som standard:** pakken leverer tilpassede prompter for `deepseek-v4-pro`, `deepseek-v4-flash`, `GLM-5.2`, `Qwen-3.7-Max`, `kimi-k2.7-code` og `kimi-k3` — når den utledede modellen matcher én av dem og ingen av dine oppføringer matcher, injiseres den innebygde prompten automatisk (dine egne filer vinner alltid: arbeidsområde > globalt > innebygd). Innebygde faner bærer et **Innebygd**-merke i modalen: rediger og lagre for å skape din egen overskriving, eller bruk fanens × for å **deaktivere** en innebygd forhåndsinnstilling (registrert i `<scope>/system_prompt/.builtin-disabled.json`). En deaktivert innebygd forhåndsinnstilling faller tilbake til Standard-filene, og oppføringen i headeren forblir synlig merket “Deaktivert”, slik at du kan aktivere den igjen når som helst.
