...
### Mode SDK (headless, `ccv -SDK`)

`ccv -SDK` exécute la session via le Agent SDK au lieu d'un terminal interactif — pas de panneau de terminal ; les messages sont envoyés depuis l'interface web, et tout le reste (journaux de session, effet machine à écrire en streaming, statistiques d'utilisation) fonctionne comme en mode terminal.

```bash
ccv -SDK                # session headless ; discutez depuis le navigateur
ccv -SDK -c             # continue la session la plus récente
ccv -SDK --model sonnet # choisissez un modèle
```

Les approbations (Bash/Edit/Write/WebFetch/…) s'affichent dans le navigateur avec autoriser / refuser / autoriser pour la session, et les invites `AskUserQuestion` / plan apparaissent sous forme de modales. `npm publish` exige toujours une approbation explicite — même avec `--d`. Nécessite le paquet `@anthropic-ai/claude-agent-sdk` (inclus avec cc-viewer) ; sinon, retombe en mode terminal.
...
* **Les préréglages intégrés sont actifs par défaut :** le paquet embarque des prompts ajustés pour `deepseek-v4-pro`, `deepseek-v4-flash`, `GLM-5.2`, `Qwen-3.7-Max`, `kimi-k2.7-code` et `kimi-k3` — lorsque le modèle résolu correspond à l'un d'eux et qu'aucune de vos entrées ne correspond, le prompt intégré est injecté automatiquement (vos propres fichiers gagnent toujours : espace de travail > global > intégré). Les onglets intégrés portent un badge **Intégré** dans la fenêtre modale : modifiez et enregistrez pour créer votre entrée de remplacement, ou utilisez le × de l'onglet pour **désactiver** un préréglage intégré (consigné dans `<scope>/system_prompt/.builtin-disabled.json`). Un préréglage intégré désactivé revient aux fichiers de l'onglet Défaut, et l'entrée d'en-tête reste visible avec la mention « Désactivé » afin de pouvoir la réactiver à tout moment.
