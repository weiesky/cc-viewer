...
* **Wbudowane ustawienia są domyślnie aktywne:** pakiet dostarcza dostrojone prompty dla `deepseek-v4-pro`, `deepseek-v4-flash`, `GLM-5.2`, `Qwen-3.7-Max`, `kimi-k2.7-code` i `kimi-k3` — gdy wyznaczony model pasuje do jednego z nich, a żaden z Twoich wpisów nie pasuje, wbudowany prompt jest wstrzykiwany automatycznie (Twoje własne pliki zawsze wygrywają: obszar roboczy > globalny > wbudowany). Wbudowane karty noszą odznakę **Wbudowany** w oknie modalnym: edytuj i zapisz, aby utworzyć własne nadpisanie, lub użyj × na karcie, aby **wyłączyć** wbudowane ustawienie (zapisywane w `<scope>/system_prompt/.builtin-disabled.json`). Wyłączone wbudowane ustawienie wraca do plików karty Domyślny, a wpis w nagłówku pozostaje widoczny z oznaczeniem „Wyłączone”, więc możesz je ponownie włączyć w dowolnym momencie.
