...
* **ビルトインプリセットはデフォルトで有効です：**パッケージには `deepseek-v4-pro`、`deepseek-v4-flash`、`GLM-5.2`、`Qwen-3.7-Max`、`kimi-k2.7-code`、`kimi-k3` 向けの調整済みプロンプトが同梱されています——解決されたモデルがいずれかに一致し、かつ自分のエントリがどれにも一致しない場合、ビルトインプロンプトが自動的に注入されます（自分のファイルが常に優先：ワークスペース > グローバル > ビルトイン）。ビルトインタブにはモーダル内に **ビルトイン** バッジが表示されます：編集して保存すればオーバーライドが作成され、タブの × を使ってビルトインを **無効** にすることもできます（`<scope>/system_prompt/.builtin-disabled.json` に記録されます）。無効化したビルトインはデフォルトタブのファイルにフォールバックし、ヘッダーのエントリは「無効」とマークされたまま表示され続けるので、いつでも再有効化できます。
