...
**音声入力**：チャット入力欄のマイクアイコンをタップすると音声認識ができます（Web Speech API。HTTPS または localhost が必要なため、LAN HTTP アクセスではボタンは表示されません）。Android では Gboard キーボード内蔵の 🎤 キーを、iOS ではシステムキーボードの音声入力機能をそのまま使えます —— どちらも HTTPS 不要、オフラインでも動作します。
