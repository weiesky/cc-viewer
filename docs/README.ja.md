...
* **デフォルト**タブ：現在のワークスペースに `CC_SYSTEM.md`（上書き）または `CC_APPEND_SYSTEM.md`（追記）を書き込み、次回の ccv 起動時に注入します。
* **モデルタブ**：名前でモデルを追加し（`opus`、`Gemini3` など）、スコープを **グローバル**（`~/.claude/cc-viewer/system_prompt/`）または **ワークスペース**（`<project>/system_prompt/`）から選びます。各タブに独自の追記/上書きスイッチとプレビューがあります。名前は解決されたモデル ID にあいまいマッチします（`opus` は `claude-opus-4-8[1m]` にマッチ）。ワークスペースがグローバルより優先、最も長い名前が勝ち、マッチしたエントリはデフォルトのファイルを完全に置き換えます。
* **ビルトインプリセット**：Kimi、DeepSeek、Qwen、GLM の各モデルファミリーに深度適合した調整済み system prompt です——解決されたモデルが一致し、かつ自分のエントリが一致しない場合に自動注入されます（自分のファイルが常に優先）。タブの × でビルトインを無効にできます。
* system テキストはセッション中のメインモデルのホットスイッチに追随し、（セッション、モデル）単位で固定されます（KV キャッシュの再構築は切り替え時の一度だけ）。タブを空で保存するとエントリを削除します。`CCV_DISABLE_AUTO_SYSTEM_PROMPT=1` ですべての自動注入を無効化でき、`CCV_DISABLE_LIVE_SYSTEM_PROMPT=1` でセッション中の追随のみを無効化できます。
