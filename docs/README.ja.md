...
* system テキストはセッション中のメインモデルのホットスイッチに追随し、（セッション、モデル）単位で固定されます（KV キャッシュの再構築は切り替え時の一度だけ）。タブを空で保存するとエントリを削除します。`CCV_DISABLE_AUTO_SYSTEM_PROMPT=1` ですべての自動注入を無効化できます。
