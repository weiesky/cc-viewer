...
- **接続拒否 (Connection Refused)**：`ccv` バックグラウンドプロセスが実行されていることを確認してください。`ccv` または `claude`（Hook インストール後）を実行すると自動的に起動するはずです。
...
- 双方向モード同期：チャットモードに切り替えると選択中のリクエストに対応する会話に自動スクロール；原文モードに戻ると選択中のリクエストに自動スクロール
