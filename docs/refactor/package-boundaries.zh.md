...
| 层 | 路径 | 允许 import |
|---|---|---|
| L0 | `findcc.js`、`server/_paths.js`、`server/i18n.js`、`src/utils/*` | L0 / L0-leaf / node builtins |
| L0-leaf | 精选纯叶子清单（检查器内 `L0_LEAVES`） | 闭包不变量：传递闭包不得越出 L0 ∪ L0-leaf ∪ builtins ∪ deps |
| L1-lib | `server/lib/**` 散文件（默认类） | L0 / L0-leaf / L1（lib+子系统）；不得引 L3/L4；引 L2 须 allowlist（R5） |
| L1-sub | `server/lib/{v2,ask,im,adapters,proxy}/` | 同 L1-lib，另加：不得跨子系统（`adapters` 与 `im` 同组——未来同一个包） |
| L2 | `server/{interceptor,proxy,pty-manager,scratch-pty-manager,workspace-registry}.js` | L0/L1/L2（目标态，gate 暂未对 L2 出边执法） |
| L3 | `server/routes/**` | L0/L1/L2（目标态，gate 暂未对 L3 出边执法） |
| L4 | `server/server.js`、`cli.js`、根 shim | 任意（组合根；仅受 R1 环约束） |
| WEB | `apps/web/src/**` | 引 packages/app 仅限 `src/utils/*` 与 L0-leaf（R6） |
...
- **R0**（硬）：L0-leaf 闭包不变量（每个叶子的传递闭包不得越出 L0 ∪ L0-leaf ∪ builtins ∪ deps）
  + 清单条目必须存在于磁盘。
- **R1**（硬）：无静态环（SCC 检测）。动态 import 单独建表；动态闭合成环须 allowlist（`R1-dyn`）。
- **R2**（硬，不可豁免）：`server/lib/**` 不得引 `server/server.js`、`routes/**`、`cli.js`。
- **R3**：跨子系统边须 allowlist。
- **R4**（硬）：L0 只引 L0 / L0-leaf / builtins。
- **R5**：L1 引 L2 须 allowlist（引 L3/L4 永远由 R2 硬拦）。
- **R6**（硬，不可豁免）：web → packages/app 仅限 `src/utils/*` + L0-leaf（传递性由 R4 的
  src/utils 纯度规则与 R0 的叶子闭包不变量共同保证）。
...
不适合拆：`findcc.js`（38 个消费者的地基）、`src/utils/`（web 双侧面缝，仅 4 文件）、
`cli.js`（组合根）、`server.js`（宿主本身）。
