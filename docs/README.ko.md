...
* **기본 제공 프리셋은 기본으로 적용됩니다:** 패키지는 `deepseek-v4-pro`, `deepseek-v4-flash`, `GLM-5.2`, `Qwen-3.7-Max`, `kimi-k2.7-code`, `kimi-k3` 용으로 튜닝된 프롬프트를 함께 제공합니다 — 해석된 모델이 이 중 하나와 일치하고 내 항목이 하나도 일치하지 않으면, 기본 제공 프롬프트가 자동으로 주입됩니다(내 파일이 항상 우선: 워크스페이스 > 전역 > 기본 제공). 기본 제공 탭은 모달에 **기본 제공** 배지를 표시합니다: 편집 후 저장하면 재정의 항목이 생성되고, 탭의 × 로 기본 제공 프리셋을 **비활성화**할 수도 있습니다(`<scope>/system_prompt/.builtin-disabled.json` 에 기록됨). 비활성화된 기본 제공 프리셋은 기본 탭의 파일로 돌아가며, 헤더 항목은 「비활성화됨」으로 표시된 채 남아 있어 언제든지 다시 활성화할 수 있습니다.
