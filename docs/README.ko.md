...
* **기본** 탭: 현재 워크스페이스에 `CC_SYSTEM.md`(덮어쓰기) 또는 `CC_APPEND_SYSTEM.md`(추가)를 기록하며, 다음 ccv 실행 시 주입됩니다.
* **모델 탭**: 이름으로 모델을 추가하고(`opus`, `Gemini3` 등) 범위를 **전역**(`~/.claude/cc-viewer/system_prompt/`) 또는 **워크스페이스**(`<project>/system_prompt/`)에서 선택합니다. 각 탭에 자체 추가/덮어쓰기 스위치와 미리보기가 있습니다. 이름은 해석된 모델 ID에 퍼지 매칭되므로(`opus`는 `claude-opus-4-8[1m]`에 매칭) 버전과 무관하게 동작합니다. 워크스페이스가 전역보다 우선하고, 가장 긴 이름이 이기며, 매칭된 항목은 기본 파일을 완전히 대체합니다.
* **기본 제공 프리셋**: Kimi, DeepSeek, Qwen, GLM 모델 패밀리에 깊이 맞춰 튜닝된 system prompt입니다 — 해석된 모델이 일치하고 내 항목이 일치하지 않으면 자동으로 주입됩니다(내 파일이 항상 우선). 탭의 × 로 기본 제공 프리셋을 비활성화할 수 있습니다.
* system 텍스트는 세션 도중 핫 스위치된 메인 모델을 따라가며 (세션, 모델) 단위로 고정됩니다(KV 캐시는 전환 시 한 번만 재구성). 탭을 비운 채 저장하면 항목이 삭제됩니다. `CCV_DISABLE_AUTO_SYSTEM_PROMPT=1`로 모든 자동 주입을 비활성화하거나, `CCV_DISABLE_LIVE_SYSTEM_PROMPT=1`로 세션 중 추적만 비활성화할 수 있습니다.
