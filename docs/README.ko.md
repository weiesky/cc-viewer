...
* system 텍스트는 세션 도중 핫 스위치된 메인 모델을 따라가며 (세션, 모델) 단위로 고정됩니다(KV 캐시는 전환 시 한 번만 재구성). 탭을 비운 채 저장하면 항목이 삭제됩니다. `CCV_DISABLE_AUTO_SYSTEM_PROMPT=1`로 모든 자동 주입을 비활성화할 수 있습니다.
