...
**语音输入**：点击聊天框的麦克风图标即可语音转文字（基于 Web Speech API；要求 HTTPS 或 localhost，LAN HTTP 访问下按钮不会出现）。Android 用户可直接使用 Gboard 键盘上的 🎤 按钮，iOS 用户可使用系统键盘自带的听写功能 —— 两者都无需 HTTPS 且支持离线。
