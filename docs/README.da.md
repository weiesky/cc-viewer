...
6. Tredjepartsmodeller understøttet: Kompatibel med deepseek-v4-\*, GLM 5.1, Kimi K2.6, med indbygget cc-switch-evne til hot-switching mellem tredjepartsværktøjer når som helst; hot-switch-dialogen understøtter også kilder pr. rolle — Main Agent, Sub-Agents og Teammates kan hver bruge en forskellig proxy-profil (standard: følg Main Agent); når Main Agent bruger det indbyggede Default med det officielle endpoint, forbliver rolletildeling skjult og inaktiv.
...
pnpm add -g cc-viewer@latest   # brug denne til opgradering — brug IKKE npm install -g til ccv installeret via pnpm
```
...
* **Modelfaner**: klik på **+ Tilføj model**, indtast et navn som `opus` eller `Gemini3`, og vælg et omfang — **Global** (`~/.claude/cc-viewer/system_prompt/`, gælder for alle arbejdsområder) eller **Arbejdsområde** (`<project>/system_prompt/`). Navnefeltet giver forslag under indtastning fra dine lokalt konfigurerede modeller (hot-reload proxy-profiler og `settings.json`); navne, der allerede er tilføjet i det valgte omfang, skjules, og vilkårlige frie navne er stadig tilladt. Hver fane har sin egen Tilføj/Overskriv-kontakt og Markdown-forhåndsvisning.
...
* **Indbyggede presets er aktive som standard:** pakken leverer tilpassede prompts til `deepseek-v4-pro`, `deepseek-v4-flash`, `GLM-5.2`, `Qwen-3.7-Max`, `kimi-k2.7-code` og `kimi-k3` — når den udledte model matcher en af dem, og ingen af dine poster matcher, injiceres den indbyggede prompt automatisk (dine egne filer vinder altid: arbejdsområde > globalt > indbygget). Indbyggede faner bærer et **Indbygget**-mærke i modalen: redigér og gem for at skabe din egen overskrivning, eller brug fanens × til at **deaktivere** en indbygget preset (registreret i `<scope>/system_prompt/.builtin-disabled.json`). En deaktiveret indbygget preset falder tilbage til Standard-filerne, og posten i headeren forbliver synlig markeret “Deaktiveret”, så du til enhver tid kan genaktivere den.
