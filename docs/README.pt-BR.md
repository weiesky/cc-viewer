...
* **Os presets integrados estão ativos por padrão:** o pacote inclui prompts ajustados para `deepseek-v4-pro`, `deepseek-v4-flash`, `GLM-5.2`, `Qwen-3.7-Max`, `kimi-k2.7-code` e `kimi-k3` — quando o modelo resolvido corresponde a um deles e nenhuma entrada sua corresponde, o prompt integrado é injetado automaticamente (seus próprios arquivos sempre vencem: espaço de trabalho > global > integrado). As abas integradas exibem um selo **Integrado** no modal: edite e salve para criar sua entrada de substituição, ou use o × da aba para **desativar** um integrado (registrado em `<scope>/system_prompt/.builtin-disabled.json`). Um integrado desativado volta aos arquivos de Padrão, e a entrada de cabeçalho permanece visível marcada como "Desativado" para que você possa reativá-la a qualquer momento.
