...
6. รองรับโมเดลของบุคคลที่สาม — รองรับ deepseek-v4-\*, GLM 5.1, Kimi K2.6 มาพร้อมความสามารถ cc-switch ในตัว สามารถสลับเครื่องมือของบุคคลที่สามได้ตลอดเวลา นอกจากนี้ การสลับแบบ hot-switch ยังรองรับแหล่งตามบทบาท — Main Agent, Sub-Agents และ Teammates สามารถใช้โปรไฟล์พร็อกซีที่แตกต่างกันได้ (ค่าเริ่มต้น: ทำตาม Main Agent) เมื่อ Main Agent ใช้ Default ในตัวกับ endpoint อย่างเป็นทางการ การกำหนดบทบาทจะถูกซ่อนและอยู่ในสถานะพัก
...
### โหมด SDK (headless, `ccv -SDK`)

`ccv -SDK` รันเซสชันผ่าน Agent SDK แทนเทอร์มินัลแบบโต้ตอบ — ไม่มีแผงเทอร์มินัล ข้อความจะถูกส่งจาก UI ของเว็บ และอย่างอื่นทั้งหมด (log เซสชัน, การพิมพ์แบบสตรีมมิ่ง, สถิติการใช้งาน) ทำงานเหมือนกับในโหมดเทอร์มินัล

```bash
ccv -SDK                # เซสชันแบบ headless; แชทจากเบราว์เซอร์
ccv -SDK -c             # ดำเนินเซสชันล่าสุดต่อ
ccv -SDK --model sonnet # เลือกโมเดล
```

การอนุมัติ (Bash/Edit/Write/WebFetch/…) จะปรากฏในเบราว์เซอร์ พร้อมตัวเลือกอนุญาต / ปฏิเสธ / อนุญาตสำหรับเซสชัน และพรอมต์ของ `AskUserQuestion` / แผนจะแสดงเป็นโมดอล `npm publish` ต้องมีการอนุมัติอย่างชัดเจนเสมอ — แม้จะใช้ `--d` ต้องใช้แพ็กเกจ `@anthropic-ai/claude-agent-sdk` (รวมมากับ cc-viewer) หากไม่มีจะกลับไปใช้โหมดเทอร์มินัลแทน
...
* แท็บ **ค่าเริ่มต้น**: เขียนไฟล์ `CC_SYSTEM.md` (เขียนทับ) หรือ `CC_APPEND_SYSTEM.md` (ต่อท้าย) ลงในเวิร์กสเปซ และจะถูกฉีดในการเปิด ccv ครั้งถัดไป
* **แท็บโมเดล**: เพิ่มโมเดลตามชื่อ (`opus`, `Gemini3`, …) พร้อมขอบเขต **ส่วนกลาง** (`~/.claude/cc-viewer/system_prompt/`) หรือ **เวิร์กสเปซ** (`<project>/system_prompt/`) แต่ละแท็บมีสวิตช์ ต่อท้าย/เขียนทับ และตัวอย่างของตัวเอง ชื่อจะถูกจับคู่แบบคลุมเครือกับรหัสโมเดลที่แปลงมา (`opus` จับคู่กับ `claude-opus-4-8[1m]`) เวิร์กสเปซชนะส่วนกลาง ชื่อที่ยาวที่สุดชนะ และรายการที่จับคู่ได้จะแทนที่ไฟล์ค่าเริ่มต้นทั้งหมด
* **พรีเซ็ตในตัว**: system prompt ที่ปรับแต่งแล้ว เข้ากันอย่างลึกซึ้งกับตระกูลโมเดล Kimi, DeepSeek, Qwen และ GLM — ฉีดโดยอัตโนมัติเมื่อโมเดลที่แปลงมาตรงกันและไม่มีรายการของคุณตรงกัน (ไฟล์ของคุณชนะเสมอ) ปิดใช้งานได้ผ่าน × บนแท็บ
* ข้อความ system จะตามการสลับโมเดลหลักแบบ hot ระหว่างเซสชัน ตรึงไว้ต่อ (เซสชัน, โมเดล) (KV-cache ถูกทำลายเพียงครั้งเดียวตอนสลับ) การบันทึกแท็บว่างจะลบรายการนั้น ตั้งค่า `CCV_DISABLE_AUTO_SYSTEM_PROMPT=1` เพื่อปิดการฉีดอัตโนมัติทั้งหมด หรือ `CCV_DISABLE_LIVE_SYSTEM_PROMPT=1` เพื่อปิดเฉพาะการติดตามระหว่างเซสชัน
