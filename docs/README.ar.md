...
- **مخرجات مختلطة (Mixed Output)**: إذا رأيت سجلات تصحيح `[CC-Viewer]` مختلطة مع مخرجات Claude، يرجى التحديث إلى أحدث إصدار (`npm install -g cc-viewer`).
- **تم رفض الاتصال (Connection Refused)**: تأكد من تشغيل عملية `ccv` في الخلفية. يجب أن يؤدي تشغيل `ccv` أو `claude` (بعد تثبيت الـ Hook) إلى تشغيله تلقائيًا.
