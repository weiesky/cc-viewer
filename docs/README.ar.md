...
* يتبع نص النظام التبديل الحار للنموذج الرئيسي أثناء الجلسة، ويُثبَّت لكل (جلسة، نموذج) (لا يُبنى KV-cache من جديد إلا مرة واحدة عند التبديل). حفظ تبويب فارغ يحذف الإدخال. عيّن `CCV_DISABLE_AUTO_SYSTEM_PROMPT=1` لتعطيل كل الحقن التلقائي.
