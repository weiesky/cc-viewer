...
#### Installazione tramite pnpm (globale)

```bash
pnpm add -g cc-viewer
pnpm add -g cc-viewer@latest   # per gli aggiornamenti — NON usare npm install -g con le installazioni pnpm
```
...
### Modalità SDK (headless, `ccv -SDK`)

`ccv -SDK` esegue la sessione tramite l'Agent SDK invece di un terminale interattivo — nessun pannello di terminale; i messaggi vengono inviati dall'interfaccia web e tutto il resto (log di sessione, effetto macchina da scrivere in streaming, statistiche di utilizzo) funziona come in modalità terminale.

```bash
ccv -SDK                # sessione headless; chatta dal browser
ccv -SDK -c             # continua la sessione più recente
ccv -SDK --model sonnet # scegli un modello
```

Le approvazioni (Bash/Edit/Write/WebFetch/…) compaiono nel browser con consenti / nega / consenti per la sessione, e le richieste `AskUserQuestion` / di piano appaiono come modali. `npm publish` richiede sempre un'approvazione esplicita — anche con `--d`. Richiede il pacchetto `@anthropic-ai/claude-agent-sdk` (incluso con cc-viewer); in caso contrario, ripiega sulla modalità terminale.
...
* **I preset integrati sono attivi per impostazione predefinita:** il pacchetto include prompt ottimizzati per `deepseek-v4-pro`, `deepseek-v4-flash`, `GLM-5.2`, `Qwen-3.7-Max`, `kimi-k2.7-code` e `kimi-k3` — quando il modello risolto corrisponde a uno di essi e nessuna tua voce corrisponde, il prompt integrato viene iniettato automaticamente (i tuoi file vincono sempre: area di lavoro > globale > integrato). Le schede integrate mostrano un badge **Integrato** nella finestra modale: modifica e salva per creare la tua voce di override, oppure usa la × della scheda per **disattivare** un integrato (registrato in `<scope>/system_prompt/.builtin-disabled.json`). Un integrato disattivato ricade sui file di Predefinito e la voce nell'intestazione resta visibile con la dicitura «Disattivato», così puoi riattivarla in qualsiasi momento.
