...
* Pestaña **Predeterminado**: escribe `CC_SYSTEM.md` (sustituir) o `CC_APPEND_SYSTEM.md` (añadir) en el espacio de trabajo, inyectado en el siguiente arranque de ccv.
* **Pestañas de modelo**: añada un modelo por nombre (`opus`, `Gemini3`, …) con ámbito **Global** (`~/.claude/cc-viewer/system_prompt/`) o **Espacio de trabajo** (`<project>/system_prompt/`); cada pestaña tiene su propio interruptor Añadir/Sustituir y vista previa. Los nombres coinciden de forma difusa con el ID de modelo resuelto (`opus` coincide con `claude-opus-4-8[1m]`); el espacio de trabajo prevalece sobre global, gana el nombre más largo, y una entrada coincidente reemplaza por completo los archivos predeterminados.
* **Preajustes integrados**: system prompts ajustados, profundamente adaptados a las familias de modelos Kimi, DeepSeek, Qwen y GLM — se inyectan automáticamente cuando el modelo resuelto coincide y ninguna entrada suya coincide (sus archivos siempre ganan). Desactive uno con la × de su pestaña.
* El texto de sistema sigue los cambios en caliente del modelo principal a mitad de sesión, fijado por (sesión, modelo) (la KV-cache solo se invalida una vez en el cambio). Guardar una pestaña vacía elimina la entrada. `CCV_DISABLE_AUTO_SYSTEM_PROMPT=1` desactiva toda inyección automática, `CCV_DISABLE_LIVE_SYSTEM_PROMPT=1` solo el seguimiento durante la sesión.
