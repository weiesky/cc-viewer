...
* El texto de sistema sigue los cambios en caliente del modelo principal a mitad de sesión, fijado por (sesión, modelo) (la KV-cache solo se invalida una vez en el cambio). Guardar una pestaña vacía elimina la entrada. `CCV_DISABLE_AUTO_SYSTEM_PROMPT=1` desactiva toda inyección automática.
