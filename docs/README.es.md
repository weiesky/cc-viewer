...
* **Los preajustes integrados están activos por defecto:** el paquete incluye prompts ajustados para `deepseek-v4-pro`, `deepseek-v4-flash`, `GLM-5.2`, `Qwen-3.7-Max`, `kimi-k2.7-code` y `kimi-k3` — cuando el modelo resuelto coincide con uno de ellos y ninguna entrada suya coincide, el prompt integrado se inyecta automáticamente (sus propios archivos siempre ganan: espacio de trabajo > global > integrado). Las pestañas integradas llevan una insignia **Integrado** en el modal: edite y guarde para crear su entrada de sustitución, o use la × de la pestaña para **desactivar** un integrado (registrado en `<scope>/system_prompt/.builtin-disabled.json`). Un integrado desactivado vuelve a los archivos de Predeterminado, y la entrada de cabecera permanece visible marcada como «Desactivado» para poder reactivarla en cualquier momento.
