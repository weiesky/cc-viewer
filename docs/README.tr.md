...
### SDK modu (headless, `ccv -SDK`)

`ccv -SDK`, oturumu etkileşimli bir terminal yerine Agent SDK üzerinden çalıştırır — terminal paneli yoktur, mesajlar web arayüzünden gönderilir ve diğer her şey (oturum logları, akışlı daktilo, kullanım istatistikleri) terminal moduyla aynı şekilde çalışır.

```bash
ccv -SDK                # headless oturum; tarayıcıdan sohbet edin
ccv -SDK -c             # en son oturuma devam et
ccv -SDK --model sonnet # bir model seç
```

Onaylar (Bash/Edit/Write/WebFetch/…) tarayıcıda izin ver / reddet / oturum için izin ver olarak açılır ve `AskUserQuestion` / plan istemleri modallar olarak görünür. `npm publish` her zaman açık bir onay gerektirir — `--d` ile bile. `@anthropic-ai/claude-agent-sdk` paketini gerektirir (cc-viewer ile birlikte gelir); kullanılamıyorsa terminal moduna döner.
...
* **Yerleşik ön ayarlar varsayılan olarak etkindir:** paket `deepseek-v4-pro`, `deepseek-v4-flash`, `GLM-5.2`, `Qwen-3.7-Max`, `kimi-k2.7-code` ve `kimi-k3` için ince ayarlanmış promptlar içerir — çözümlenen model bunlardan biriyle eşleştiğinde ve hiçbir girdiniz eşleşmediğinde, yerleşik prompt otomatik olarak enjekte edilir (kendi dosyalarınız her zaman kazanır: çalışma alanı > genel > yerleşik). Yerleşik sekmeler modalda **Yerleşik** rozeti taşır: kendi üzerine yazmanızı oluşturmak için düzenleyip kaydedin veya yerleşik bir ön ayarı **devre dışı bırakmak** için sekmenin × işaretini kullanın (kaydı `<scope>/system_prompt/.builtin-disabled.json` içinde tutulur). Devre dışı bırakılan yerleşik ön ayar Varsayılan dosyalara geri döner ve başlıktaki girdi görünür kalır, “Devre dışı” olarak işaretlenir — böylece istediğiniz zaman yeniden etkinleştirebilirsiniz.
