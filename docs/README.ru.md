...
`ccv -SDK` запускает сессию через Agent SDK вместо интерактивного терминала — без панели терминала; сообщения отправляются из веб-интерфейса, а всё остальное (журналы сессии, потоковая печать, статистика использования) работает так же, как в режиме терминала.

```bash
ccv -SDK                # headless-сессия; чат из браузера
ccv -SDK -c             # продолжить последнюю сессию
ccv -SDK --model sonnet # выбрать модель
```

Запросы на подтверждение (Bash/Edit/Write/WebFetch/…) появляются в браузере с вариантами allow / deny / allow-for-session, а `AskUserQuestion`/запросы плана отображаются в виде модальных окон. `npm publish` всегда требует явного подтверждения — даже с `--d`. Требуется пакет `@anthropic-ai/claude-agent-sdk` (входит в состав cc-viewer); при его отсутствии используется режим терминала.
...
* **Встроенные пресеты активны по умолчанию:** пакет содержит настроенные промпты для `deepseek-v4-pro`, `deepseek-v4-flash`, `GLM-5.2`, `Qwen-3.7-Max`, `kimi-k2.7-code` и `kimi-k3` — когда определённая модель совпадает с одной из них и ни одна из ваших записей не совпадает, встроенный промпт внедряется автоматически (ваши собственные файлы всегда в приоритете: рабочая область > глобально > встроенный). Встроенные вкладки отмечены значком **Встроенный** в модальном окне: отредактируйте и сохраните, чтобы создать вашу переопределяющую запись, или нажмите × на вкладке, чтобы **отключить** встроенный пресет (записывается в `<scope>/system_prompt/.builtin-disabled.json`). Отключённый встроенный пресет возвращается к файлам «По умолчанию», а запись в заголовке остаётся видимой с пометкой «Отключено», чтобы вы могли снова включить её в любой момент.
