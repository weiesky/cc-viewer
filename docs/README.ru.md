...
* Системный текст следует за горячим переключением основной модели посреди сессии, фиксируется для пары (сессия, модель) (KV-кэш сбрасывается только один раз при переключении). Сохранение пустой вкладки удаляет запись. `CCV_DISABLE_AUTO_SYSTEM_PROMPT=1` отключает любое автоматическое внедрение.
