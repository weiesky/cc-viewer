...
- **NPM Install**: Автоматически внедряет скрипты перехвата в `cli.js` Claude Code.
...
Если вам нужно использовать пользовательскую конечную точку API (например, корпоративный прокси), просто настройте ее в `~/.claude/settings.json` или установите переменную окружения `ANTHROPIC_BASE_URL`. `ccv` автоматически распознает это и корректно перешлет запросы.

### Тихий режим (Silent Mode)

По умолчанию `ccv` работает в тихом режиме при запуске `claude`, обеспечивая чистоту вывода терминала, идентичную оригинальному опыту Claude Code. Все логи захватываются в фоновом режиме и доступны для просмотра на `http://localhost:7008`.
...
- **Смешанный вывод (Mixed Output)**: Если вы видите отладочные логи `[CC-Viewer]` вперемешку с выводом Claude, пожалуйста, обновитесь до последней версии (`npm install -g cc-viewer`).
