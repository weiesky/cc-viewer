...
* Вкладка **По умолчанию**: записывает `CC_SYSTEM.md` (переопределение) или `CC_APPEND_SYSTEM.md` (добавление) в рабочую область; внедряется при следующем запуске ccv.
* **Вкладки моделей**: добавьте модель по имени (`opus`, `Gemini3`, …) с областью **Глобально** (`~/.claude/cc-viewer/system_prompt/`) или **Рабочая область** (`<project>/system_prompt/`); у каждой вкладки свой переключатель «Добавить/Переопределить» и предпросмотр. Имена сопоставляются нечётко с определённым ID модели (`opus` совпадает с `claude-opus-4-8[1m]`); рабочая область имеет приоритет над глобальной, побеждает самое длинное имя, совпавшая запись полностью заменяет файлы по умолчанию.
* **Встроенные пресеты**: настроенные system prompt, глубоко адаптированные под семейства моделей Kimi, DeepSeek, Qwen и GLM — внедряются автоматически, когда определённая модель совпадает, а ни одна из ваших записей не совпадает (ваши файлы всегда в приоритете). Отключение через × на вкладке.
* Системный текст следует за горячим переключением основной модели посреди сессии, фиксируется для пары (сессия, модель) (KV-кэш сбрасывается только один раз при переключении). Сохранение пустой вкладки удаляет запись. `CCV_DISABLE_AUTO_SYSTEM_PROMPT=1` отключает любое автоматическое внедрение, `CCV_DISABLE_LIVE_SYSTEM_PROMPT=1` — только отслеживание во время сессии.
