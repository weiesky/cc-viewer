...
- ปิดตามค่าเริ่มต้น — ตั้ง `CLAUDE_CODE_REPL=true` เพื่อเปิดใช้งาน
- ในเซสชันเทอร์มินัล (`cli`) และ claude.ai (`remote`) feature flag ฝั่ง server อาจเปิดใช้งานได้เช่นกัน
- เมื่อปิดอยู่ REPL จะถูกซ่อนจาก tool list ของโมเดล เมื่อเปิดอยู่ `Read`, `Glob`, `Grep`, `Bash`, `PowerShell`, และ `NotebookEdit` จะถูกแทนที่ด้วยชวเลขของ REPL
